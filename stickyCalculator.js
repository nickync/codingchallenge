...
function stickyCalc (operation, val1, val2){
    ...
    switch (operation) {
        case '+':
          return Math.round(Number((Math.round(val1)).toString().concat(val2)) + val2)
          break
        ...
        case '*':
          return Math.round(Number(val1.toString().concat(val2)) * val2)
          break 
        case '/':
          return  Math.round(Number(val1.toString().concat(val2)) / val2)
          break
...
// Best Solution
function stickyCalc (oper, val1, val2){
    const round = x => Math.round(x)
    const first = String(round(val1)) + String(round(val2))
    return oper === '+' ? Number(first) + round(val2) :
            oper ==='-' ? Number(first) - round(val2) :
            oper ==='*' ? Number(first) * round(val2) :
            round( Number(first) / round(val2) );
}